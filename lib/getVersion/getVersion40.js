exports.getVersion40 = function (bufferString,callback) {
	'use strict';
	...
	while ((beginVcard = bufferString.indexOf('BEGIN:VCARD', endVcard)) != -1) {
		...
		for (var i=0 ; i<contactString.length ; ++i) {
			...
			if (contactString[i].substr(0,5) === 'PHOTO') {
				TYPE = contactString[i].substring(contactString[i].indexOf('MEDIATYPE')+10 , contactString[i].indexOf(':',contactString[i].indexOf('MEDIATYPE')));
			...
			if (contactString[i].substr(0,3) === 'ADR') {
				TYPE = contactString[i].substring(9, contactString[i].indexOf(';',9));
				ADR = contactString[i].substring(contactString[i].indexOf('"')+1, contactString[i].lastIndexOf('"'));
				ADR = escape(ADR);
